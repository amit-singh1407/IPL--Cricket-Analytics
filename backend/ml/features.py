...
from backend.utils.db import serialize_document
...
def _coerce_float(value: Any, default: float = 0.0) -> float:
    if isinstance(value, dict):
        if "runs" in value:
            return _coerce_float(value.get("runs"), default)
        if "value" in value:
            return _coerce_float(value.get("value"), default)
        return default
...
def build_score_feature_frame(scorecards: list[dict[str, Any]], repository):
    ...
    for scorecard in scorecards:
        ...
        targets.append(_coerce_float(total_runs))
